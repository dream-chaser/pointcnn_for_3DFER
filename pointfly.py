...
def batch_normalization(data, is_training, name, reuse=None):
    return tf.layers.batch_normalization(data, momentum=0.9, training=is_training,
                                         beta_regularizer=tf.contrib.layers.l2_regularizer(scale=regular_factor),
                                         gamma_regularizer=tf.contrib.layers.l2_regularizer(scale=regular_factor),
...
def separable_conv2d(input, output, name, is_training, kernel_size, depth_multiplier=1,
    ...
    conv2d = tf.layers.separable_conv2d(input, output, kernel_size=kernel_size, strides=(1, 1), padding='VALID',
                                        ...
                                        depthwise_regularizer=tf.contrib.layers.l2_regularizer(scale=regular_factor),
                                        pointwise_regularizer=tf.contrib.layers.l2_regularizer(scale=regular_factor),
...
def conv2d(input, output, name, is_training, kernel_size,
    ...
    conv2d = tf.layers.conv2d(input, output, kernel_size=kernel_size, strides=(1, 1), padding='VALID',
                              ...
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=regular_factor),
                              reuse=reuse, name=name)
    return batch_normalization(conv2d, is_training, name, reuse) if with_bn else conv2d
