...
def main():
    ...
    parser.add_argument('--save_folder_chenzhixing', '-s', help='Path to folder for saving check points and summary', required=True)
    ...
    root_folder = os.path.join(args.save_folder_chenzhixing, '%s_%s_%s' % (args.model, args.setting, args.train_name))
    ...
    data_train, label_train, data_val, label_val, verif_idx = setting.load_fn(args.path, args.path_val)
    ...
    data_val_placeholder = tf.placeholder(data_val[0].dtype, (1, None, data_val[0].shape[-1]))
    label_val_placeholder = tf.placeholder(label_val[0].dtype, (1,))
...
#    iterator_val = dataset_val.make_initializable_iterator()
    ...
    (pts_fts, labels) = tf.cond(is_training, lambda: iterator.get_next(), lambda: (data_val_placeholder, label_val_placeholder))
...
#        exit()
        ...
        for batch_idx_train in range(batch_num):
            ######################################################################
            # Validation
            if t_1_acc > 0.3 and ((batch_idx_train != 0 and batch_idx_train % step_val == 0) or batch_idx_train == batch_num - 1):

                filename_ckpt = os.path.join(folder_ckpt, 'last_model')
                saver.save(sess, filename_ckpt, global_step=None)
                print('{}-Checkpoint saved to {}!'.format(datetime.now(), filename_ckpt))

                data_verif = data_val[:verif_idx]
                label_verif = label_val[:verif_idx]
                data_zip = list(zip(data_val[verif_idx:], label_val[verif_idx:]))
                sam_num = 500
                data_sam = random.sample(data_zip, sam_num)
                data_total = list(zip(data_verif, label_verif)) + data_sam
                label_sam = []
                for ds in data_sam:
                  label_sam.append(ds[1])
                label_sam = np.squeeze(np.array(label_sam).astype(np.int32))
                ...
                for pre_i in range(predict_num):
                    ...
                    valid_i_feats = []
                    for batch_idx_val in range(len(data_total)):
                        batch_size_val = 1
                        xforms_np, rotations_np = pf.get_xforms(batch_size_val, rotation_range=rotation_range_val,
                                                                    order=setting.order)
                        data_input = data_total[batch_idx_val][0]
                        label_input = data_total[batch_idx_val][1]
                        _, out_feats = \
                            sess.run([update_ops, feats_mean],
                                     feed_dict={
                                         data_val_placeholder: data_input,
                                         label_val_placeholder: label_input,
                                         handle: handle_train,
                                         indices: pf.get_indices(batch_size_val, sample_num, point_num),#, False),
                                         xforms: xforms_np,
                                         rotations: rotations_np,
                        ...
                        valid_i_feats.append(out_feats)
                    valid_feats.append(valid_i_feats)

                valid_feats = np.array(valid_feats)
                feats = np.mean(valid_feats.reshape((valid_feats.shape[0], valid_feats.shape[1], valid_feats.shape[3])), axis=0)
                feats = feats / np.sqrt(np.sum(feats**2, axis=1, keepdims=True))
                dot_mat = np.dot(feats[verif_idx:], feats[:verif_idx].T)
                predict_label = np.argmax(dot_mat, axis=1)
                true_num = np.count_nonzero(predict_label == label_sam)
                t_1_acc_avg = true_num / predict_label.shape[0]
                print('{}-[Val  ]-Average:      T-1 Acc: {:.4f}'
                      .format(datetime.now(), t_1_acc_avg))
                if t_1_acc_avg > 0.8:
                  filename_ckpt = os.path.join(folder_ckpt, '%d_model_%f' % (batch_idx_train, t_1_acc_avg))
                  saver.save(sess, filename_ckpt, global_step=None)
                  print('{}-Checkpoint saved to {}!'.format(datetime.now(), filename_ckpt))
                if t_1_acc_avg > (best_acc-1e-6):
                  best_acc = t_1_acc_avg
                  print('{}-[Val  ]-best:         T-1 Acc: {:.4f}'
                      .format(datetime.now(), t_1_acc_avg))
...
#                sys.stdout.flush()
            ...
            _, loss, t_1_acc, summaries = \
                sess.run([train_op, loss_op, t_1_acc_op, summaries_op],
                         feed_dict={
                             data_val_placeholder: data_val[0],
                             label_val_placeholder: label_val[0],
                             handle: handle_train,
