...
def load_cls_BU3D(filelist):
    ...
    for line in open(filelist):
        filename = line.rstrip()
        data = h5py.File(os.path.join(folder, filename))
        if 'normal' in data and 'shape_index' in data:
            input_x = np.concatenate([data['data'][...], data['data'][...], data['normal'][...], data['shape_index'][...]], axis=-1).astype(np.float32)
            input_x = np.transpose(input_x, (1, 0, 2))
            input_x = np.random.permutation(input_x)
            input_x = np.transpose(input_x, (1, 0, 2))
            points.append(input_x)
        else:
            points.append(data['data'][...].astype(np.float32))
        labels.append(data['label'][...][0].astype(np.int32))
    return (np.concatenate(points, axis=0),
            np.concatenate(labels, axis=0))


def load_cls_train_val_BU3D(filelist, filelist_val):
    data_train, label_train = grouped_shuffle(load_cls_BU3D(filelist))
    data_val, label_val = load_cls_BU3D(filelist_val)
    return data_train, label_train, data_val, label_val


def load_verification(filelist):
    points = []
    labels = []

    for lid, line in enumerate(open(filelist)):
        if lid % 100 == 0:
            print('train_data', lid)
            sys.stdout.flush()
        filename = line.rstrip()
        ...
        if 'normal' in data and 'shape_index' in data:
            ...
            input_x = np.concatenate([data['data'][...], data['data'][...]], axis=-1).astype(np.float32)
        input_x = np.transpose(input_x, (1, 0, 2))
        input_x = np.random.permutation(input_x)
        input_x = np.transpose(input_x, (1, 0, 2))
        points.append(input_x)
...
def load_verification_val(filelist):
    points = []
    labels = []
    ...
    verif_lines = [line for line in lines if line.endswith('N_N_0.mat')]
    verif_lines.sort()
    valid_lines = [line for line in lines if not line.endswith('N_N_0.mat')]
    valid_lines.sort()
    verif_idx = len(verif_lines)
    lines = verif_lines + valid_lines
    for lid, filename in enumerate(lines):
        if lid % 100 == 0:
            print('valid_data', lid)
            sys.stdout.flush()
        data = h5py.File(filename)
        if 'normal' in data and 'shape_index' in data:
            input_x = np.concatenate([data['data'][...], data['data'][...], data['normal'][...], data['shape_index'][...]], axis=-1).astype(np.float32)
        ...
        points.append(input_x)
        labels.append(data['label'][...][0].astype(np.int32))
    return points, labels, verif_idx


def load_verification_train_val(filelist, filelist_val):
    data_train, label_train = grouped_shuffle(load_verification(filelist))
    data_val, label_val, verif_idx = load_verification_val(filelist_val)
    return data_train, label_train, data_val, label_val, verif_idx
