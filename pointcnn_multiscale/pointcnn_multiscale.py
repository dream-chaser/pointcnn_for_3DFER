...
def xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation, depth_multiplier,
    ...
    if with_X_transformation:
        ...
        X_2 = pf.dense(X_0, K * K, tag + 'X_2', is_training, with_bn=False, activation=None)
...
class PointCNN:
    def __init__(self, points, features, num_class, is_training, setting, task):
        ...
        if features is None:
            ...
            C_fts = 0
            for xp in xconv_params[0]:
                C_fts += xp[-1]
            C_fts = C_fts // 2
            features_hd = pf.dense(features, C_fts, 'features_hd', is_training)
            self.layer_fts = [features_hd]
            self.layer_out.append(features_hd)

        for layer_idx, layer_param_multiscale in enumerate(xconv_params):
            layer_P = None
            layer_C = 0
            for scale_idx, layer_param in enumerate(layer_param_multiscale):
                K, D, P, C = layer_param
                if layer_P is None:
                    layer_P = P
                elif layer_P != P:
                    raise Exception('Different P in the same layer.')
                layer_C += C
            ...
            if layer_idx == 0:
                C_pts_fts = layer_C // 2 if fts is None else layer_C // 4
                depth_multiplier = 4
            else:
                C_prev = 0
                for xp in xconv_params[layer_idx - 1]:
                    C_prev += xp[-1]
                C_pts_fts = C_prev // 4
                depth_multiplier = math.ceil(layer_C / C_prev)

            fts_xconv = []
            for scale_idx, layer_param in enumerate(layer_param_multiscale):
                tag = 'xconv_' + str(layer_idx + 1) + '_' + str(scale_idx + 1) + '_'
                K, D, P, C = layer_param

                fts_scale_i = xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation,
                                  depth_multiplier, sorting_method)
                fts_xconv.append(fts_scale_i)
            fts_xconv = tf.concat(fts_xconv, axis=2, name=tag + 'fts_concat')
            self.layer_fts.append(fts_xconv)
            self.layer_out.append(fts_xconv)
        ...
        if task == 'classification':
            ...
            self.feats_mean = tf.reduce_mean(self.fc_layers[-1], axis=1, name='feats_mean')
